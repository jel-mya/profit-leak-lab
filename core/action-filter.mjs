...
export function filterActions(actions, view, asOf) {
  if (!['All actions', 'Unfinished', 'Closed', 'Overdue', 'Needs details'].includes(view)) throw new Error('Unknown action view.');
  ...
  return actions.filter(action => {
    ...
    if (view === 'Needs details') {
      ...
      if (!action.owner?.trim()) return true;
      try { dateValue(action.due); return false; } catch { return true; }
    }
