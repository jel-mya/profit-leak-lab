...
Recovery form drafts are separate session-only state. Filtering or collapsing cards does not discard them. Unsaved amount, date and evidence never enter this download; saving updates the action recovery and revision history, while Discard restores the saved record for that action. Clearing the session, restoring the fictional demo, or applying the first private import clears the corresponding demo actions and drafts.
