...
## Run101 recovery draft continuity integration - 12 September 2026

- Fetched canonical main before changes: e059e73cabb099eb115cb2454ba43ea54ce9170d, matching the run101 base. Verified all six package SHA-256 entries. The controlled-draft feature was missing; integrated the run101 core/declarations, 17 focused tests and adapted page changes. Older run94/run95/run96 handoffs were not applied.
- Recovery amount/date/evidence now live in per-action session drafts across all filters and tab unmounts. Hidden dirty drafts contribute to the existing exit-warning condition and count. Save clears only the successfully saved action draft; failed validation retains it. Discard restores the last saved fields. Action resets clear drafts as well.
- Reused existing recovery validation, history/revision logic, status/evidence rules, saved-state action download, filters, investigation calculations and beforeunload listener. No duplicate implementations. Existing clear/demo-restore behaviour is preserved; canonical has no action-download restore parser, and none was introduced.
