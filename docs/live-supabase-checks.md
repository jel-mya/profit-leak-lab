...
### Recovery verification in write mode

The opted-in write command now extends its newly created synthetic action through recovery revision 3 and a zero-value correction at revision 4. Apply all migrations, including `202609120001_action_recovery.sql`, first. It reads the synthetic business currency, verifies foreign/viewer/anonymous denial and currency mismatch, races two recovery writes at the same revision, checks stale retry denial, then compares the full recovery audit snapshots. Only fixed pass/fail labels are logged. A failed run may leave partial synthetic history; use a disposable project. This harness has local fault-injection coverage but has not been run against live Supabase. Revocation, currency changes during a write and browser interactions remain separate gates.
